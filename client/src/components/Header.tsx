        ...
        <div className="flex items-center gap-3">
          <div className="items-center rounded-full border border-[#DAB383]/80 bg-[#F5F0E8] px-3 py-2 text-sm text-[#3E2A1D] flex">
            ...
            <input
              ...
              className="w-36 bg-transparent text-sm text-[#3E2A1D] outline-none placeholder:text-[#3E2A1D]/45"
            />
          </div>

          <button
            onClick={onCartClick}
            className="relative flex items-center justify-center rounded-full border border-[#DAB383] bg-[#E8DDCC] p-2.5 text-[#3E2A1D] transition-transform hover:-translate-y-0.5"
            aria-label="Open cart"
          >
            <ShoppingBag size={17} />
            {cartCount > 0 && (
              <span className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-[#754E2E] text-[10px] font-bold text-[#F5F0E8]">
                {cartCount}
              </span>
            )}
          </button>
